...
Profile.prototype.__init = function(programOptions, packageRootPath, parentSMCore) {
	var self = this;
	self.programOptions = programOptions;

	self.parentSMCore = parentSMCore;
...
Profile.prototype.getCredentials = function(ns) {
	...
	if (this.parentSMCore) {
		var parentCredentials = this.parentSMCore.getCredentials(ns);
		if (parentCredentials && UTIL.len(parentCredentials) > 0) {
			credentials = UTIL.deepMerge(parentCredentials, credentials);
		}
	}
	return credentials;
}
