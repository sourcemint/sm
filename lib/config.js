		...
		if (process.env.SM_BIN_PATH) {
			...
			function resolve(binPath) {
				...
				if (
					...
					homeBasePath = exports.makeAbsolutePath(PATH.dirname(descriptorPath), descriptor.config["github.com/sourcemint/sm/0"].toolchain.paths.home);
				} else {
					return next();
				}
			}
			...
			if (API.FS.existsSync(programDescriptorPath)) {
				try {
					...
					if (descriptor.has(SM_NS.concat(["toolchain", "paths", "home"]))) {
						return exports.makeAbsolutePath(PATH.dirname(descriptor.file), descriptor.get(SM_NS.concat(["toolchain", "paths", "home"])));
					}
				} catch(err) {}
			}
...
Config.prototype.makeAbsolutePath = function(basePath, path) {
	return exports.makeAbsolutePath(basePath, path);
}
...
exports.makeAbsolutePath = function (basePath, path) {
