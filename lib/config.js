...
Config.prototype.__init = function(program) {
	...
	return API.Q.call(function() {
			...
			API.FS.writeFileSync(programDescriptorPath, JSON.stringify({
				config: {
					"github.com/sourcemint/sm/0": {
       					...
       					"profile": "default",
   						"paths": {
							"home": self.makeRelativePath(API.PATH.dirname(API.PATH.dirname(self.basePath)))
						}
					}
				}
			}, null, 4));
		...
		var homePath = self.makeAbsolutePath(self.programDescriptor.get(SM_NS.concat(["paths", "home"])));
		...
		[
			...
			if (!self.programDescriptor.has(SM_NS.concat(["paths", dir]))) {
				self.programDescriptor.set(
					SM_NS.concat(["paths", dir]),
					self.makeRelativePath(API.PATH.join(homePath, dir))
				)
			}
			var path = self.makeAbsolutePath(self.programDescriptor.get(SM_NS.concat(["paths", dir])));
...
Config.prototype.get = function(ns) {
	return this.programDescriptor.get(SM_NS.concat(ns));
}
