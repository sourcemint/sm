...
//console.log("MAKE LOCATOR FOR", descriptor, new Error().stack);
...
//			    		if (options.debug) console.log("non-fatal error", err.stack);
		...
		if (typeof descriptor.pointer !== "undefined") {
			...
			// HACK: This needs major cleanup.
			// Get toolchain catalog to remap pointer if applicable.
			var originalPointer = descriptor.pointer;
			var catalog = node.getCore().getCatalog();
			if (catalog) {
				var newPointer = catalog.resolvePointer(descriptor.pointer);
				if (newPointer !== descriptor.pointer) {
					descriptor.pointer = newPointer;
				} else if (descriptor.name) {
					newPointer = node.getCore().getCatalog().resolvePointer(descriptor.name + "@" + descriptor.pointer);
					if (newPointer !== (descriptor.name + "@" + descriptor.pointer)) {
						descriptor.pointer = newPointer;
					}
				}
				if (newPointer && /^\//.test(newPointer)) {
					// Avoid circular links.
					if (node.path === newPointer) {
						descriptor.pointer = originalPointer;
					} else {
						descriptor.pm = "symlink";
					}
				}
			}
