...
Locator.prototype.equals = function(locator) {
	if (this.rev && locator.rev && this.rev === locator.rev) return true;
...
throw new Error("Display location!");
/*
    	if (this.location !== false) return this.location;
    	if (this.locations && this.locations.original) return this.locations.original;
    	return "[invalid locator: " + JSON.stringify(this) + "]";
*/
    }
}
Locator.prototype.getLatest = function(node, options, callback) {
	try {
		...
		if (node.latest[self.descriptor.pm]) {
			latest = node.latest[self.descriptor.pm];
		}
...
Locator.prototype.getLatestSatisfying = function(node, options, callback) {
	...
	function findBestSatisfying(versions, selector, loadDescriptor, callback) {
		...
		var version = false;
		var versionIndex = -1;
		var platformVersion = node.summary.newPlatformVersion || node.summary.platformVersion || false;
		function iterate(callback) {
			...
			if (!node.summary.platformName || !platformVersion) {
			...
			function check(descriptor) {
				...
				if (
					!descriptor ||
					!descriptor.engines ||
					!descriptor.engines[node.summary.platformName] ||
					SEMVER.satisfies(platformVersion, descriptor.engines[node.summary.platformName])									
				) {
					return callback(null, version);
				}
	...
	function checkVCS(callback) {
		...
		function checkSelector(callback) {
		    return node.getPlugin(node.summary.vcs.type).then(function(plugin) {
		    	return plugin.descriptorForSelector(self, self.selector, options).then(function(info) {
		    		var versions = {};
		    		versions[self.selector] = info.descriptor;
		    		return findBestSatisfying(versions, self.selector, null, function(err, found) {
		    			if (err) return callback(err);
		    			if (!found) {
		    				TERM.stdout.writenl("\0yellow([sm] WARNING: `engines` declared in latest 'package.json' at '" + self.selector + "' is not compatible with your 'engine'!\0)");
...
//console.log(node.summary.vcs.type, node.latest[node.summary.vcs.type]);
		...
		if (node.latest[node.summary.vcs.type] && node.latest[node.summary.vcs.type].versions) {
			var lastRev = false;
    		return findBestSatisfying(node.latest[node.summary.vcs.type].versions, self.selector, function(version, callback) {
			    return node.getPlugin(node.summary.vcs.type).then(function(plugin) {
	...
	try {
		return checkVCS(function(err, locator) {
			...
			if (!node.latest[self.descriptor.pm] || !node.latest[self.descriptor.pm].versions) return callback(null, false);
    		return findBestSatisfying(node.latest[self.descriptor.pm].versions, self.selector, null, function(err, found) {
				if (!found) return callback(null, false);
				return callback(null, makeLocator(false, found));
    		});
		});
	} catch(err) {
		return callback(err);
