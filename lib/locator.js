...
//		    		if (options.debug) console.log("non-fatal error", err.stack);
		    		return resolveViaArchive(callback);
		    	}
	    		return resolve(pluginId, plugin, function(err, locator) {
	    			if (err) {
//			    		if (options.debug) console.log("non-fatal error", err.stack);
			    		return resolveViaArchive(callback);
	    			}
...
Locator.prototype.isOlderThan = function(node, locator, options, callback) {
	...
	if (node.summary.vcs) {
	    return node.getPlugin(node.summary.vcs.type, function(err, plugin) {
	    	if (err) return callback(err);
	    	// NOTE: This does not really test `isOlderThan`. Only in the sense that if it is
	    	//		 found in the history it is older than current.
	    	// TODO: Implement `plugin.isRevDescendant(parentRev1, descRev2) === true`.
	    	return plugin.hasRevInHistory(locator.rev, options, function(err, found) {
	    		if (err) return callback(err);
				return callback(null, !found);
	    	});
	    });
	} else if (node.summary.rev) {
	    return node.getPlugin(node.summary.rev.pm, function(err, plugin) {
	    	if (err) return callback(err);
	    	// NOTE: This does not really test `isOlderThan`. Only in the sense that if it is
	    	//		 found in the history it is older than current.
	    	// TODO: Implement `plugin.isRevDescendant(parentRev1, descRev2) === true`.
