...
var Catalog = exports.Catalog = function(path, descriptor, options) {
	...
	if (options.format === "list") {
		self.packages = descriptor[options.packagesAttribute || "packages"] || {};
	} else
	if (options.format === "tree") {
        ...
        processNode(descriptor[options.packagesAttribute || "dependencies"], "");
	} else {
		throw new Error("Unknown catalog descriptor format '" + options.format + "'");
	}

    if (descriptor.intercept) {
        for (var rule in descriptor.intercept) {
            self.intercept.push({
                re: new RegExp(rule),
                package: descriptor.intercept[rule]
            });
        }
    }

    self.resolvePointer = function(pointer) {
        for (var i=0 ; i<self.intercept.length ; i++) {
            if (self.intercept[i].re.exec(pointer)) {
                var pkg = self.lookupPackage(self.intercept[i].package);
                if (pkg) {
                    if (pkg.pointer) return pkg.pointer;
                    if (pkg.location) return pkg.location;
                    if (pkg.archive) return pkg.archive;
                }
            }
        }
        return pointer;
