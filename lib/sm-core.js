		...
		return self.status(opts).then(function(node) {
			return node[0].diff(options).then(function(changes) {
				...
				for (var packagePath in changes) {
					if (changes[packagePath] && UTIL.len(changes[packagePath]) > 0) {
	                    ...
	                    TERM.stdout.writenl("Package: \0bold(" + packagePathString + "\0)");
	                    for (var relpath in changes[packagePath]) {
		                    TERM.stdout.writenl("  " + relpath + " \0yellow(" + changes[packagePath][relpath] + "\0)");
	                    }
	                }
				}
			});
