	...
	self.resolve = function(id, options) {
		...
		try {
			function load() {
				...
				options.logger.debug("Resolved resource id `" + id + "` to path `" + path + "`");

				if (path) {
					if (!PATH.existsSync(path)) {
						throw new Error("Resource for id '" + id + "' not found at path '" + path + "'.");
					}
	...
	self.require = function(id, options) {
		...
		try {
			function load() {
				...
				options.logger.debug("Resolved module id `" + id + "` to path `" + path + "`");

				if (path) {
					if (!PATH.existsSync(path)) {
						throw new Error("Module for id '" + id + "' not found at path '" + path + "'.");
					}
					var inst = null;
					try {
