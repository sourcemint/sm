...
var SMCore = function(packageRootPath, parentSMCore) {
	...
	scanner.onNewNode = function(node) {
		...
		node.getCachePath = function(type, uri) {
			var path = false;
			var cacheBasePath = self.getConfig(["toolchain", "paths", "cache"]);
			if (type === "install") {
				path = PATH.join(
					cacheBasePath,
					"install",
	...
	self.init = function(uri, options) {
		...
		return config.reinit().then(function() {
			...
			FS.outputFileSync(PATH.join(packageRootPath, ".sm", "source.json"), JSON.stringify({
				locator: {
					pointer: uri
				}
			}));
...
console.warn("TODO: Rename new toolchain `name` in `package.json` to `dirname(packageRootPath)` and reset `version`");
	...
	self.switch = function(options) {
		...
		return self.status(opts).then(function(nodes) {
			...
			if (options.stopWorkspace) {
				...
				var homeBasePath = CONFIG.getHomeBasePath(true);
				var smBinPath = CONFIG.getSmBinPath(homeBasePath);

				var pinfParentPaths = [];

				var toolchainConfig = new CONFIG.for(packageRootPath);
				return toolchainConfig.__init(options, packageRootPath).then(function() {

					var done = Q.resolve();

					(toolchainConfig.get(["toolchain", "extends"]) || []).forEach(function(toolchainPath) {
						done = Q.when(done, function() {
							var path = CONFIG.makeAbsolutePath(toolchainConfig.basePath, toolchainPath);
							pinfParentPaths.push(path + "/bin");
							pinfParentPaths.push(path + "/.sm/bin");
							if (!smBinPath) {
								smBinPath = CONFIG.getSmBinPath(path);
							}
						});
					});
					...
					return done.then(function() {
						...
						if (pinfParentPaths.length > 0) {
							pinfParentPaths.push("");
						}
						activateTpl = activateTpl.replace(/__PINF_PARENT_PATH__/g, pinfParentPaths.join(":"));

						activateTpl = activateTpl.replace(/__SM_HOME__/g, homeBasePath);
						activateTpl = activateTpl.replace(/__SM_BIN_PATH__/g, smBinPath);

						var extraCommands = [];
						var env = self.getCredentials(["env"]);
						if (env) {
							for (var key in env) {
							    extraCommands.push('export ' + key + '="' + env[key] + '"');
							}
						}
// TODO: Instead of hardcoding these into the activation file we should be calling a script that will read the env vars
//       and return them in a delimited fashion allowing bash to export them. We can then reduce the security level for the activation file.
//       There should be a helper lib for bash as part of the pinf project.
						activateTpl = activateTpl.replace(/___EXTRA_COMMANDS___/g, extraCommands.join("\n"));
