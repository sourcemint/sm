...
var SMCore = function(packageRootPath) {
	...
	var homeBasePath = PATH.join(process.env.HOME, ".sm");

	var profile = PROFILE.for(PATH.join(homeBasePath, "profiles", "default"));
	...
	scanner.onNewNode = function(node) {
		node.getPlugin = function(pluginName) {
			return self.getPlugin(API, node, pluginName);
		}
