...
var SMCore = function(packageRootPath) {
	...
	var credentials = CREDENTIALS.for("default");
...
*/
	...
	self.init = function(uri, options) {
		...
		FS.mkdirSync(PATH.join(packageRootPath, ".sourcemint"));
		FS.writeFileSync(PATH.join(packageRootPath, ".sourcemint", "source.json"), JSON.stringify({
			locator: {
				pointer: uri
			}
		}));
...
throw new Error("Switch to cloned repo.");
