...
var SMCore = function(packageRootPath) {
	...
	self.status = function(options) {
		return scanner.fsTree(options).then(function(tree) {
			...
			if (!tree.exists) {
                TERM.stdout.writenl("\0red([sm] ERROR: Path '" + tree.path + "' does not exist!\0)");
			...
			return self.status(opts).then(function(list) {
		        ...
		        node.getPlugin("git", function(err, plugin) {
		        	...
		        	if (uri) {
			            ...
			            return edit(node.summary.declaredLocator).then(deferred.resolve, deferred.reject);
		        	}
		        });
		        return deferred.promise;
			});
	...
	self.bump = function(options) {
		...
		return self.status(opts).then(function(topPackage) {
			var deferred = Q.defer();
	        topPackage.getPlugin(topPackage.summary.pm.locator, function(err, plugin) {
	        	if (err) return deferred.reject(err);
...
// TODO: Only call if `node.hints.actions.publish` is set. (not dirty, etc...)
			...
			if (topPackage.summary.vcs) {
					...
					var deferred = Q.defer();
					topPackage.getPlugin(topPackage.summary.vcs.type, function(err, pm) {
			        	if (err) return deferred.reject(err);
			...
			return Q.when(done, function() {
	        	...
	        	if (topPackage.summary.scripts.publish) {
					...
					return Q.when(done, function() {
						var deferred = Q.defer();
						topPackage.getPlugin(topPackage.summary.pm.publish, function(err, pm) {
							if (err) return deferred.reject(err);
							if (topPackage.descriptor.package.publish === true) {
				                TERM.stdout.writenl("Running publish for pm: " + pm.pluginId);
								return pm.publish(options).then(deferred.resolve, deferred.reject);
							} else {
				                TERM.stdout.writenl("\0yellow(Skipping publish via pm '" + pm.pluginId + "' as `package.json ~ publish: true` is not set.\0)");
							}
							return deferred.resolve();
						});
						return deferred.promise;
		...
		return self.status(options).then(function(tree) {
			...
			return Q.fcall(function() {
				if (UTIL.isArrayLike(tree)) {
					if (tree.length > 1) {
						throw new Error("Matched more than one package for selector '" + selector + "'!");
					}
					...
					return node.install(options).then(function() {
						installedNodes.push(node);
						node.print();
					});
					...
					return traverse(tree).then(function() {
						...
						return tree.install(options).then(function() {
							tree.print();
				...
				installedNodes.forEach(function(node) {
	                    ...
	                    return node.postinstall(options);
					});
				});
	...
	self.run = function(options) {
		...
		options.levels = 0;
		return self.status(options).then(function(topPackage) {
			if (topPackage.descriptor.package &&
				topPackage.descriptor.package.scripts &&
				topPackage.descriptor.package.scripts.run
			) {
				// TODO: Refactor to generic script runner module.
				// e.g. `./node_modules/app-node-webkit/bin/nw .`.
				var command = null;
				if (/^\.\//.test(topPackage.descriptor.package.scripts.run)) {
					// We want to run a script relative to package.
					command = PATH.join(topPackage.path, topPackage.descriptor.package.scripts.run);
				} else {
					// e.g. `node server.js`
					command = topPackage.descriptor.package.scripts.run;
				}
				var opts = UTIL.copy(options);
				opts.cwd = topPackage.path;
				opts.returnOutput = true;
				return OS.spawnInline(command.split(" ")[0], command.split(" ").slice(1), opts);
//                TERM.stdout.writenl("\0red(ERROR: Unable to call script '" + topPackage.descriptor.package.scripts.run + "'.\0)");
//	            throw true;
			} else {
