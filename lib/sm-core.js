...
var SMCore = function(packageRootPath, parentSMCore) {
	...
	self.__init = function(program) {
		return Q.fcall(function() {
			return config.__init(program, packageRootPath).then(function() {
				...
				return profile.__init(program, packageRootPath, parentSMCore);
			});
		});
	}
		...
		return self.status(opts).then(function(topPackage) {
	        ...
	        topPackage.getPlugin(topPackage.summary.pm.locator, function(err, plugin) {
				...
				if (!topPackage.hints.actions.bump) {
                    TERM.stdout.writenl("");
                    TERM.stdout.writenl("  \0yellow(Cannot bump. Is VCS dirty/ahead/behind?\0)");
                    TERM.stdout.writenl("");
                    return deferred.reject(true);
				}
	...
	self.run = function(options) {
		...
		return self.status(opts).then(function(nodes) {
			if (nodes[0].descriptor.package &&
				...
				opts.env.PINF_PROGRAM = PATH.join(opts.cwd, "program.json");
				opts.env.PINF_PACKAGE = PATH.join(opts.cwd, "package.json");
				opts.env.PINF_RUNTIME = PATH.join(opts.cwd, ".rt/program.rt.json");
				opts.env.PINF_MODE = opts.mode || process.env.PINF_MODE || "production";
