...
//	    	"dry-run": options["dry-run"] || false,
	...
	program
		...
		*/
		...
		.option("--init-toolchain [templateUri]", "Initialize a new toolchain from template.")
...
//        		callArgs: options.callArgs
	...
	program
        ...
        .action(function() {
        	calling = true;
        	core.publish(makeOptions()).then(callPromise.resolve, callPromise.reject);
        });
		...
		function respond(args, expanded) {
			...
			return core.__init(program).then(function() {
				...
				if (program.initToolchain) {
					core.init(
						(program.initToolchain === true) ? "github.com/freedom-platform/dev" : program.initToolchain,
						makeOptions()
					).then(function() {
...
function installCommand() {
	...
	return Q.call(function() {
        ...
        TERM.stderr.writenl("\0red([sm] ERROR: Installing `" + COMMAND_NAME + "` in your PATH at '" + commandPath + "'! Try re-running with `\0bold(sudo " + COMMAND_PATH + " --install-command\0)`. `sudo` is only needed to install the `sm` command, not when you call `sm` later.\0)");
        throw true;
	});
