	...
	self.respond = function(args) {
		function respond(args, expanded) {
			...
			try {
				...
				if (!calling) {
					if (!expanded && program.rawArgs.length > 2 && program.rawArgs[2] === "s") {
						var args = UTIL.copy(program.rawArgs);
						args[2] = "status";
						respond(args, true).then(callPromise.resolve, callPromise.reject);
					} else {
						program.help();
					}
				}
