...
FsNode.prototype.initForPath = function(path, options, callback) {
    ...
    function populateLocator() {
        ...
        if (self.parent) {
            if (self.parent.descriptors.package.mappings && (locator.pointer = findDependency(self.parent.descriptors.package.mappings))) {
                locator.viaAttribute = "mappings";
                normalizeMapping(locator);
            } else
            if (self.parent.descriptors.package.devMappings && (locator.pointer = findDependency(self.parent.descriptors.package.devMappings))) {
                locator.viaAttribute = "devMappings";
                normalizeMapping(locator);
            } else
            if (self.parent.descriptors.package.optionalMappings && (locator.pointer = findDependency(self.parent.descriptors.package.optionalMappings))) {
                locator.viaAttribute = "optionalMappings";
                locator.optional = true;
                normalizeMapping(locator);
            } else
            if (self.parent.descriptors.package.dependencies && (locator.pointer = findDependency(self.parent.descriptors.package.dependencies))) {
                // TODO: Only default to `npm` if we are running on nodejs. Otherwise leave empty.
                locator.pm = "npm";
                locator.viaAttribute = "dependencies";
            } else
            if (self.parent.descriptors.package.devDependencies && (locator.pointer = findDependency(self.parent.descriptors.package.devDependencies))) {
                // TODO: Only default to `npm` if we are running on nodejs. Otherwise leave empty.
                locator.pm = "npm";
                locator.viaAttribute = "devDependencies";
            } else
            if (self.parent.descriptors.package.optionalDependencies && (locator.pointer = findDependency(self.parent.descriptors.package.optionalDependencies))) {
                // TODO: Only default to `npm` if we are running on nodejs. Otherwise leave empty.
                locator.pm = "npm";
                locator.viaAttribute = "optionalDependencies";
            }
            if (self.parent.descriptors.package.bundleDependencies && findDependency(self.parent.descriptors.package.bundleDependencies)) {                
                // TODO: Only default to `npm` if we are running on nodejs. Otherwise leave empty.
	                        ...
	                        waitForPackages(function(done) {
		                        ...
		                        FS.lstat(path, function(err, stat) {
		                            ...
		                            function initChild() {
		                            	...
		                            	return node.initForPath(path, opts, done);
		                            }
