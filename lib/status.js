    	...
    	fsTree.traverse(function(node) {
    		...
    		waitFor(function(done) {
    		    return loadStatus(node, options, function(err) {
                    ...
                    // TODO: Abstract this out into `waitfor` package.
                    var queue = [];
                    function enqueue(callback) {
                        queue.push(callback);
                    }
                    node.on("waitfor", enqueue);

                    return summarize(node, options, function(err) {
                        if (err) return done(err);

                        node.removeListener("waitfor", enqueue);
        ...
        waitFor(function(done) {
            return node.getPlugin(pair[0]).then(function(plugin) {
                return plugin.latest(options).then(function(latest) {
                    node.latest[pair[0]] = latest || false;
                });
            }).then(done, done);
        });
    });
...
//            git: false,
        ...
        if (!info.pm.install) info.pm.install = "sm";
...
throw new Error("TODO: Get `pointer` by looking at bundled child.");
        ...
        return LOCATOR.makeLocator(node, locator, options, function(err, locator) {
            ...
            if (info.installed) {
                info.actualLocator = info.declaredLocator.clone();
            ...
            if (info.symlinked === "inside" || info.inParent) {
                info.declared = true;
                if (info.descriptors.locator.bundled) {
                    info.bundled = true;
                }
            }
            ...
            if (node.descriptors.smSource && node.descriptors.smSource.platformVersion) {
                info.platformVersion = node.descriptors.smSource.platformVersion;
...
//console.log("node", node.name, node.latest);
        ...
        function determineLocked(callback) {
            ...
            if (info.locked) {
                ...
                locator.pm = info.declaredLocator.descriptor.pm;
        ...
        function determineSticky(callback) {
            ...
            if (info.sticky) {
                ...
                return LOCATOR.makeLocator(node, locator, options, function(err, locator) {
                    if (err) return callback(err);
                    info.sticky = locator;
                    if (!info.actualLocator || !info.sticky.equals(info.actual)) {
...
// TODO: Move padding to bottom.
        ...
        if (info.pm.locator) {
            segment = " \0" + ((hints.actions.install)?"red":"green") + "(" + info.pm.locator;
...
*/
        ...
        if (info.vcs) {
            ...
            if (info.declaredLocator.selector && info.vcs.selector && info.declaredLocator.selector === info.vcs.selector) {
                line.push("\0orange(" + info.vcs.selector + "\0) - " + info.vcs.rev);
            } else {
                line.push(info.vcs.selector + " - " + info.vcs.rev);
            }
            if (info.vcs.version) {
