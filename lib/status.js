	...
	self.embellishFsTree = function(fsTree, options) {
    	...
    	var waitFor = WAITFOR.parallel(function(err) {
            ...
            var waitFor = WAITFOR.parallel(function(err) {
                ...
                try {
                    if (typeof fsTree.collectDeepHints === "function") {
                        fsTree.collectDeepHints();
                    }
                } catch(err) {
...
throw new Error("TODO: Get `pointer` by looking at bundled child.");
        ...
        return LOCATOR.makeLocator(node, locator, options, function(err, locator) {
            ...
            info.engineVersion = process.version;
            if (node.descriptors.smSource && node.descriptors.smSource.engineVersion) {
                info.engineVersion = node.descriptors.smSource.engineVersion;
                if (info.engineVersion !== process.version) {
                    info.newEngineVersion = process.version;
                }
            }
...
//                updateOptional: false
    ...
    /*
            info.newLocator = info.newLockedLocator || 
                              ...
                              ((!info.installed || info.newEngineVersion) && info.locator);
    */
        if (!info.installed) {
            if (info.locked) {
