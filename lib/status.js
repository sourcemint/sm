        ...
        if (node.summary.pm.locator && typeof node.latest[node.summary.pm.locator] === "undefined") {
            return node.getPlugin(node.summary.pm.locator).then(function(plugin) {                
                return plugin.latest(options).then(function(latest) {
                    node.latest[node.summary.pm.locator] = latest || false;
                });
    ...
    Object.keys(node.status).forEach(function(pluginName) {
        waitFor(function(done) {
            return node.getPlugin(pluginName).then(function(plugin) {
                return plugin.latest(options).then(function(latest) {                    
                    node.latest[pluginName] = latest || false;
                });
            }).then(done, done);
        });
    });
...
function addFunctions(node, options) {
    ...
    node.refresh = function(options) {
        ...
        node.initForPath(node.path, options, function(err) {
            if (err) return deferred.reject(err);
            return exports.for(node.path).embellishFsTree(node, options).then(function() {
                return node;
...
callback(new Error("TODO: Force download VCS in cache."));
        ...
        var deferred = Q.defer();
        isVcsCacheAvailable(options.vcsOnly, function(err, cachePath) {
            if (cachePath) {

console.log("cachePath", cachePath);

return Q.reject(new Error("TODO: Extract from VCS cache"));
                ...
                [
                    ...
                    waitFor(function(done) {
                        if (cacheInfo.status !== 404) return done();
                        ...
                        return node.getPlugin(type).then(function(plugin) {
                            ...
                            return plugin.download(uri, options).then(function(response) {
                                ...
                                return plugin.extract(response.cachePath, installCachePath, options).then(function() {
                                    cacheInfo = {
                                        status: 200,
                                        pointer: uri,
                                        cachePath: installCachePath
    ...
    node.install = function(options) {
        ...
        var instructions = node.hints.actions.install;
        ...
        try {
            if (instructions[0] === "code") {
                ...
                options.logger.info("Installing `" + node.path + "`");
