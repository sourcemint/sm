			...
			if (pair[1] === true) {
				...
				waitFor(function(done) {
					return node.getPlugin(pair[0], function(err, plugin) {
                        ...
                        if (options.debug) console.log("[sm] Load status for '" + node.path + "' via plugin '" + pair[0] + "'");
    ...
    var waitFor = WAITFOR[((options.debug)?"serial":"parallel")](function (err) {
        ...
        if (node.summary.pm.locator && typeof node.latest[node.summary.pm.locator] === "undefined") {
            return node.getPlugin(node.summary.pm.locator, function(err, plugin) {                
                ...
                if (options.debug) console.log("[sm] Load latest for '" + node.path + "' via plugin '" + node.summary.pm.locator + "'");
    ...
    Object.keys(node.status).forEach(function(pluginName) {
        waitFor(function(done) {
            return node.getPlugin(pluginName, function(err, plugin) {
                ...
                if (options.debug) console.log("[sm] Load latest for '" + node.path + "' via plugin '" + pluginName + "'");
