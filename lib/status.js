...
function summarize(node, options, callback) {
    try {
        ...
        node.descriptor = {
            package: node.descriptors.package || false,
    ...
    //            git: false,
            ...
            return LOCATOR.makeLocator(node, locatorPointer, options, function(err, locator) {
                ...
                function formatPointers(callback) {
                    ...
                    function formatHomepageUri() {
                        if (!info.homepageUri) return;
                    ...
                    return Q.when(formatHomepageUri(), function() {
                        return Q.when(formatUid(), function() {
                            return callback(null);
                        });
                    }).fail(callback);
                }
        ...
        function mergeExtends(node, callback) {
            ...
            if (!node.descriptors.package || !node.descriptors.package.extends) {
                return callback(null);
            }
            ...
            function merge(alias, callback) {
                ...
                return mergeExtends(node.children[alias], function(err) {
                    ...
                    node.descriptor.package = UTIL.deepMerge(node.children[alias].descriptors.packagePrototype, node.descriptor.package);

                    if (node.descriptor.package.extends.length > 0) {
                        return merge(node.descriptor.package.extends.pop());
                    }
                    delete node.descriptor.package.extends;
                    return callback(null);
                });
            }
            return merge(node.descriptor.package.extends.pop(), callback);
        }

        return mergeExtends(node, function(err) {
            if (err) return callback(err);
...
// TODO: Populate `info.dynamic` from `.sm/source.json` ~ dynamic if installed via `SM.install()`.
                if (!info.dynamic) {
                    if (info.level <= 1) {
...
*/
        function isVcsCacheAvailable(force, callback) {
                ...
                return function(options) {
                    return node.getPlugin(latest.type).then(function(plugin) {
                        ...
                        if (options.vcsOnly !== true) {
                            ...
                            if ((PATH.existsSync(genericCachePath) && (installCachePath = genericCachePath)) || PATH.existsSync(installCachePath)) {
            ...
            if (force) {
                ...
                return node.getPlugin("git").then(function(plugin) {
                    var opts = UTIL.copy(options);
                    opts.forceClone = true;
                    return plugin.latest(opts).then(function(latest) {
                        return makeExtractor(latest);
                    });
        ...
        try {
            ...
            if (options.link) {
                return Q.call(function() {
                    ...
                    if (!skipLinking) {
                        if (!PATH.existsSync(PATH.dirname(nodePath))) {
                            FS_RECURSIVE.mkdirSyncRecursive(PATH.dirname(nodePath));
                        }
                    ...
                    if (node.parent) {
                        return node.refresh(options).then(function(node) {
                            return node.getPlugin(node.summary.pm.install).then(function(plugin) {
                                return plugin.postinstall(node, options);
                            });
                        });
                    } else {
            ...
            if (instructions[0] === "top-package" && instructions[1] === "install") {
                ...
                return node.getPlugin(node.summary.pm.install).then(function(plugin) {
                    ...
                    return plugin.install(node.path, options);
                });
                ...
                return node.downloadAndExtract(instructions[1], options).then(function(response) {
                    ...
                    return node.getPlugin(node.summary.pm.install).then(function(plugin) {
                        ...
                        if (response.status === 200 && options.vcsOnly) {
                            return copyVcsOnly().then(function() {
                                if (PATH.existsSync(cachePath)) {
                                    FS_RECURSIVE.rmSyncRecursive(cachePath);
                                }
                            });
                        }
                        ...
                        if (response.status === 200) {
                                ...
                                options.logger.debug("Asking package manager `" + node.summary.pm.install + "` to install package.");
                                return plugin.install(cachePath, options).fail(function(err) {
                                    if (PATH.existsSync(cachePath)) {
                                        FS_RECURSIVE.rmSyncRecursive(cachePath);
                                    }
                                    throw err;
                                    ...
                                    if (PATH.existsSync(PATH.join(node.path, "package.json"))) {
                                        try {
                                    ...
                                    if (!engineVersionSpecific) {
                                        ...
                                        if (!PATH.existsSync(genericCachePath)) {
                                            if (!PATH.existsSync(PATH.dirname(genericCachePath))) {
                                                FS_RECURSIVE.mkdirSyncRecursive(PATH.dirname(genericCachePath));
                                            }
                        ...
                        return Q.when(done, function() {
                            ...
                            if (options.vcsOnly) {
                                ...
                                if (PATH.existsSync(node.path) && options.noBackup !== true) {
                                ...
                                return FS_RECURSIVE.osCopyDirRecursive(cachePath, node.path).then(function() {
                            ...
                            PATH.exists(PATH.join(node.path, ".sm"), function(exists) {
                                if (!exists) {
                            ...
                            return node.refresh(options).then(function(node) {
                                return plugin.postinstall(node, options).then(function() {
                                    return node;
                                });
                            });
                        });
